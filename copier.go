...
type Option struct {
    ...
    IgnoreEmpty bool
    IgnoreNotCheckBool bool
    IgnoreNotCheckInt bool
    ...
    for i := 0; i < amount; i++ {
        ...
        if source.IsValid() {
            ...
            for _, field := range fromTypeFields {
                ...
                if fromField := source.FieldByName(srcFieldName); fromField.IsValid() && !shouldIgnore(fromField, opt.IgnoreEmpty, opt.IgnoreNotCheckBool, opt.IgnoreNotCheckInt) {
            ...
            for _, field := range deepFields(toType) {
                ...
                if fromMethod.IsValid() && fromMethod.Type().NumIn() == 0 && fromMethod.Type().NumOut() == 1 && !shouldIgnore(fromMethod, opt.IgnoreEmpty, opt.IgnoreNotCheckBool, opt.IgnoreNotCheckInt) {
...
func shouldIgnore(v reflect.Value, ignoreEmpty bool, IgnoreNotCheckBool bool, IgnoreNotCheckInt bool) bool {
    if !ignoreEmpty {
        return false
    }
    if IgnoreNotCheckBool {
        if v.Type().Kind() == reflect.Bool {
            return false
        }
    }
    if IgnoreNotCheckInt {
        kind := v.Type().Kind()
